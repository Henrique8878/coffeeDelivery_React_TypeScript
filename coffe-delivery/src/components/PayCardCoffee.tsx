...
export function PayCardCoffee(){
    ...
    return(
        <>
            {objectCoffee?objectCoffee.map((coffee)=>(
            <div key={coffee.id} className={coffee.coffeeQuantity!=0?'flex justify-between w-[100%] pb-6 h-[100px] border-b border-base-button':'hidden'}>
                        <img src={coffee.coffeePicture} alt="" />
                        <div className='flex flex-col gap-2'>
                            <span className='Text-M font-bold text-base-subtitle'>{coffee.coffeeTitle}</span>
                            <div className='flex gap-2'>
                                <ButtonAddCoffee valueQuantity = {coffee.coffeeQuantity} coffeeID ={coffee.id}/>
                                <ButtonRemoveCoffee coffeeID={coffee.id} valueQuantity={coffee.coffeeQuantity}/>
                            </div>
